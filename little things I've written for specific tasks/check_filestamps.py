...
Check timestamp of each file
Compile dictionary of items with timestamps
'''
...
def get_timestamps(vs, vs_auth, item_id):
	...
	for file in shape_files:
		timestamps.append(file.find('timestamp').text)
	return timestamps
...
def main(environment,proxy_config_file,script_file_name):
	...
	resultsfile = open('timestamps.txt', 'w')
	item_list = open('itemIds.txt', 'r')
	for item in item_list:
		timestamp_dict = {}
		item_id = item.strip()
		timestamp_dict[item_id.strip()] = get_timestamps(vs,vs_auth,item_id.strip())
		print(timestamp_dict)
		print('\n')
		resultsfile.write(str(timestamp_dict))
		resultsfile.write('\n')
	return True
