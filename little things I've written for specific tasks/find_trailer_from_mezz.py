...
def compile_candidate_list(vs,vs_auth,indab_items):
    '''parse list of items to find HD 15.0 trailers'''
    candidate_list = []
...
def compile_sd_candidate_list(vs,vs_auth,indab_items):
    '''parse list of items to find SD trailers'''
    sd_candidate_list = []
    for item in indab_items:
        # subtype/trailer check
        subtype = get_group_metadata_value(vs,vs_auth,item,'file_information_subtype_descriptor')
        is_trailer = get_group_metadata_value(vs,vs_auth,item,'file_information_is_trailer')
        if subtype and "CL_SD" in subtype and is_trailer.lower() == 'true':
            # in house/corrupt check
            corrupt = get_group_metadata_value(vs,vs_auth,item,'media_management_corrupt')
            shape_presence = get_shape_file(vs, vs_auth, item)
            if (corrupt and corrupt.lower() == 'true') or shape_presence is None:
                continue
            print(f'Item {item} is a qualified SD derivative.')
            sd_candidate_list.append(item)
    return sd_candidate_list
...
def main(environ,proxy_config_path,script_file):
    ...
    resultsfile = open('trailer_check_250911.csv', 'w+', encoding='utf-8')
    resultsfile.write('Old Trailer ID, New Trailer ID,Trailer Checksum,Trailer Filename\r')
    ...
    for item in item_list:
        ...
        if not indab_master_id or indab_master_id == '0':
            ...
            resultsfile.write(f'{item},N/A,N/A,N/A\r')
            continue
        ...
        if len(candidate_list) == 0:
            print(f'No HD deriv trailers found for trailer mezz {item}.\n')
            sd_candidate_list = compile_sd_candidate_list(vs,vs_auth,indab_items)
            if len(sd_candidate_list) == 0:
                print(f'No SD deriv trailers found for trailer mezz {item}.\n')
                resultsfile.write(f'{item},N/A,N/A,N/A\r')
                continue
            candidate_list = sd_candidate_list
        print(f'{len(candidate_list)} eligible trailer(s) found for trailer item {item}.')
        print(f'Trailer ID: {max(candidate_list)}.')
        checksum = get_group_metadata_value(vs,vs_auth,max(candidate_list),
                                                    'original_shape_mi_original_shape_mi_md5_hash')
        filename = get_system_metadata_value(vs,vs_auth,max(candidate_list),
                                                     'originalFilename')
        print(f'{filename}\n{checksum}\n')
        resultsfile.write(f'{item},{max(candidate_list)},{checksum},{filename}\r')
    print('Done.')
