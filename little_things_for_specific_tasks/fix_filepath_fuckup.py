...
Locate VX-266 file
If uri contains "%0D%0A", move file to new path without that substring
'''
...
def get_local_file_path(vs, vs_auth, item_id):
	...
	for file in shape_files:
		if file.find('storage').text == 'VX-266':
			...
			if "%0D" in file_path:
				corrected_file_path = file_path.replace('%0D','')
				return file_id, corrected_file_path
			else:
				print(f'File {file_id} does not contain %0D in the path')
				return False, False
...
def main(environment,proxy_config_file,script_file_name):
	...
	for item in item_list:
		file_id, corrected_file_path = get_local_file_path(vs, vs_auth, item.replace('\n',''))
		if file_id:
			move_file(vs, vs_auth, file_id, corrected_file_path)
	return True
