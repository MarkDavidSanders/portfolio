...
-Gets Vault secrets
-Constructs and sends POST call to start backing up the item's file
-Returns job ID
'''
...
logger.info(f'{item_id} provided as item id.', extra=extras)
...
def start_backup(vs, token):
    url = f'{vs}API/job?type=ind_backup&jobmetadata=itemId={item_id}'
...
def main():
    ...
    if not vs_token_data:
        ...
        exit(2)
    job_id = start_backup(vs, vs_token_data['token'])
    logger.info(f'backup job started. job id created: {job_id}')
