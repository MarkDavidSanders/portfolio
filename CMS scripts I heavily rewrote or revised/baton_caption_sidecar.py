...
def update_metadata_success(vs_token_data, results, item_id):
	# update metadata and return a list of dicts
...
def update_metadata_fail(vs_token_data, result, item_id):
	fields = ['shift', 'drift', 'incorrect', 'missing', 'duration', 'line_length', 'line_count', 'reading_rate']
	group = 'sidecar_caption_analysis'
	for field in fields:
		f = f'{group}_{field}'
...
def main():
	...
	if failed and args['template'] not in metadata_skip_templates:
		vs_token_data = get_vs_token()
		update_metadata_fail(vs_token_data, msg, item_id)
		exit(0)

	success_values = extract_success_values(report)
	vs_token_data = get_vs_token()
	if args['template'] not in metadata_skip_templates:
		results_list = update_metadata_success(vs_token_data, success_values, item_id)
	...
	if correction:
		...
		logger.warning(f'Downloading Corrected Captions for {sidecar_caption_id}')
		download_corrected_captions(scc_path, scc_linux, sidecar_caption_id, api_url, job, subtask_id, bc_token)
