...
-Gets Vault secrets
-Constructs and sends POST call to start up a VS proxy job
...
# let's do it
def start_proxy(vs, token):
    url = f'{vs}API/job?type=ind_proxy&priority=LOWEST&jobmetadata=itemId={item_id}'
...
def main():
    ...
    if not vs_token_data:
        ...
        exit(2)
    job_id = start_proxy(vs, vs_token_data['token'])
