...
script_version = "250204.19"
log_level = "INFO" # DEBUG INFO WARN ERROR
'''
ARGUMENTS RECEIVED FROM VANTAGE:
1. VS Parent File ID
2. VS Deletion Lock Document (with escape quotes)
3. Environment

WHAT THIS SCRIPT DOES:
-Gets Vault secrets
-Constructs and sends POST call to add a deletion-lock to the file
-Errors out if response code bad
'''
...
logger.info(f'{file_id} provided as parent file id.', extra=extras)
logger.info(f'deletion lock document: {metadata}')
...
# let's do it
def start_backup(vs, token):
    url = f'{vs}API/file/{file_id}/deletion-lock'
    headers = {
    ...
    payload = metadata
    try:
        response = requests.post(url, headers=headers, data=payload, verify=crt_file)
...
def main():
    ...
    if not vs_token_data:
        ...
        exit(2)
    status_code = start_backup(vs, vs_token_data['token'])
    logger.info(f'post status code {status_code}')
    return True
